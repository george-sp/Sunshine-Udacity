...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    public static final String ACTION_DATA_UPDATED = "com.example.android.sunshine.ACTION_DATA_UPDATED";
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
        ...
        // We no longer need just the location String,
        // but also potentially the latitude and longitude,
        // in case we are syncing based on a new Place Picker API result.
        ...
        try {
            ...
            // we want to potentially build a query using a lat/lon value.
            // This will be the case when we are syncing based off of a new location
            // from the Place Picker API.
            // So we need to check if we have a lat/lng to work with,
            // and use those when we do.
            // Otherwise, the weather service may not understand the location address
            // provided by the Place Picker API and the user could end up with no weather! The horror!
            if (Utility.isLocationLatLonAvailable(context)) {
                uriBuilder.appendQueryParameter(LAT_PARAM, locationLatitude)
                        .appendQueryParameter(LNG_PARAM, locationLongitude);
            } else {
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            if (forecastJson.has(OWN_MESSAGE_CODE)) {
                int errorCode = Integer.parseInt(forecastJson.getString(OWN_MESSAGE_CODE));
                switch (errorCode) {
                    case HttpURLConnection.HTTP_OK:
                        break;
                    case HttpURLConnection.HTTP_NOT_FOUND:
                        setLocationStatus(getContext(), LOCATION_STATUS_INVALID);
                        // If break; used instead of return; You are going for debugging.
                        return;
                    default:
                        setLocationStatus(getContext(), LOCATION_STATUS_SERVER_DOWN);
                        return;
    ...
    /**
     * Sets the location status into SharedPreferences.
     * This function should not be called from the UI thread
     * because it uses commit to write the SharedPreferences.
     *
